        ...
        <div class="flex">
            <div class="w-1/4">
                <ul class="w-60 flex flex-col divide-y divide-gray-200 ">
                    @foreach($here as $member)

                        <li class="inline-flex items-center gap-x-2 py-3 text-sm font-medium text-gray-900 "> {{$member['name']}} </li>
                    @endforeach
                </ul>
            </div>
            <div class="w-3/4">
                    ...
                    @foreach($this->messages as $msg)
                        ...
                        @if($msg['id'] == auth()->user()->id )
                            <div class="flex gap-2.5 justify-end pb-40">
                                <div class="">
                                    <div class="grid mb-2">
                                        <h5 class="text-right text-gray-900 text-sm font-semibold leading-snug pb-1">{{$msg['author']}}</h5>

                                        <div class="px-3 py-2 bg-gray-100 rounded">
                                            <h2 class="text-gray-900 text-sm font-normal leading-snug">{{$msg['message']}}</h2>
                                        </div>

                                        <div class="justify-start items-center inline-flex">
                                            <h3 class="text-gray-500 text-xs font-normal leading-4 py-1">{{$msg['date']}}</h3>
                                        </div>
                                    </div>
                                </div>
                        ...
                        @if($msg['id'] !== auth()->user()->id  )
                            <div class="flex gap-2.5 mb-4">
                             {{--   <img src="https://pagedone.io/asset/uploads/1710412177.png" alt="Shanay image" class="w-10 h-11">--}}
                                <div class="grid">
                                    <h5 class="text-gray-900 text-sm font-semibold leading-snug pb-1"> {{$msg['author']}}</h5>
                                    <div class="w-max grid">
                                        <div class="px-3.5 py-2 bg-indigo-600 rounded justify-start  items-center gap-3 inline-flex">
                                            <h5 class="text-white text-sm font-normal leading-snug"> {{$msg['message']}}</h5>
                                        </div>
                                        <div class="justify-end items-center inline-flex mb-2.5">
                                            <h6 class="text-gray-500 text-xs font-normal leading-4 py-1">{{$msg['date']}}</h6>
                                        </div>
                                    </div>
                                </div>
                            </div>
